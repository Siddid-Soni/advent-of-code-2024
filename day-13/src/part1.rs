...
pub fn process<P>(filename: P) -> u32 where P: AsRef<Path>  {
    ...
    let mut total = 0;
    for block in &lines {
        let nums = block.iter().map(|x| re.find_iter(x).map(|x| x.as_str().parse::<f64>().unwrap()).collect::<Vec<_>>()).collect::<Vec<_>>();
...
mod tests {
    ...
    #[test]
    fn test_process() {
        assert_eq!(480, process("src/test.txt"));
    }
}